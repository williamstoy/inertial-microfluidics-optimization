...
for i = 1:length(d)
    fprintf(1, 'Loading %s\n', d(i).name);
    [shape_db, all_stat_db, data_db, data_std_db] = display_flow_progress(d(i).name, shape_db, all_stat_db, data_db, data_std_db, 0);
end
...
function [shape_db, all_stat_db, data_db, data_std_db] = display_flow_progress(filename, shape_db, all_stat_db, data_db, data_std_db, fileoffset)
    min_val = 83*.2;
    max_val = 83*2.1;
    ...
    re = str2double(fnamefrag{9}(3:end-4));
    
    shape_id = str2double([num2str(width) num2str(height)]);
    
    if sum(shape_db == shape_id) == 0
        shape_db(end+1, :) = [shape_id, width, height];
    end
    ...
    if mod(size(data,1),100) == 0
        ...
        d = dir(sprintf('sim_params_cw%d_ch%d_*_nh%d_*_re%d*', width, height, notch_height, floor(re)));
        ...
        c = 1-(re-min_val)/(max_val-min_val);
        color = [0 c c];
        ...
        fill([xdata'; flipud(xdata')], [focus_quality - focus_quality_std; flipud(focus_quality + focus_quality_std)], color, 'FaceAlpha', 0.4, 'LineStyle', 'none', 'HandleVisibility', 'off');
        lgd = legend();
        lgd.String{end} = sprintf('Re: %.2f \\mum, V_{mean}=%.2f[m/s]', re, data(end,5));
        sgtitle(sprintf('Width: %d\\mum, Height: %d\\mum, Notch Height: %d', width, height, notch_height));
