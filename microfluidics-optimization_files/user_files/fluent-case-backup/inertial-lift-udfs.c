...
void CALCULATE_CHANNEL_PARAMETERS(double particle_diameter, double mu, double rho, double *H, double *W, double *Re, double *kappa, double particle_position[3])
{
	...
	Q = RP_Get_Real("volumetric_flow_rate_ul_per_min_scheme"); // uL/min
  ...
  *H = channel_height;
    ...
    // we need to shift the center offset down due to the presence of the notch
		channel_center_offset_y = notch_height/2; // notch comes from the top so the center position shifts down
...
DEFINE_ON_DEMAND(port_input_vars_to_scheme)
{
  ...
  double channel_width, channel_height, channel_length, notch_height, notch_length;
...
DEFINE_DPM_INJECTION_INIT(particle_init,I)
{
  ...
  loop(p,I->p)  // Standard ANSYS FLUENT Looping Macro to get particle streams in an Injection
  {
    ...
    if (count > max_count) {
      ...
      y = ((H-d) * get_random() + d/2.0) - H/2.0;
      z = ((W-d) * get_random() + d/2.0) - W/2.0;
