...
DEFINE_DPM_INJECTION_INIT(particle_init,I)
{
  ...
  // set zexclude to 2 to put particles in the whole channel inlet
  double zexclude = 2;
  ...
  loop(p,I->p)  // Standard ANSYS FLUENT Looping Macro to get particle streams in an Injection
  {
    ...
    if (count > max_count) {
      ...
      // only put the particles in the outer 2/zexclude of the channel
      r = get_random();
      y = ((H-d) * r + d/2) - H/2;
      r = get_random();
